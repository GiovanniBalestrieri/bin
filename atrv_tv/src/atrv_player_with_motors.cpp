...
public:
  ...
  ros::Publisher power_pub;     // ST ST ST
  ...
  AtrvNode() {
    ...
    power_pub = node_.advertise<std_msgs::Float64>("battery_power", 1);  // ST ST ST
  ...
  int start() {
    ...
    mPower = new PowerProxy(mPlayer,0);  // ST ST ST
  ...
  void doUpdate() {
    ...
    if(mPower->IsValid()) {   // ST ST ST
		std_msgs::Float64 current_power;
		current_power.data = mPower->GetCharge();
...
int main(int argc, char** argv) {
	...
	while (n.ok()) {
	  ros::spinOnce();
	  if((++counter)%100==0) ROS_INFO("I'm alive %ld",counter);
	  atrv.doUpdate();
